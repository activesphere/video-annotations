...
const ImageNodeView = (node: ProsemirrorNode<typeof EditorState>, view: EditorView<typeof EditorState>, getPos: () => number) => {
  ...
  return {
    selectNode: () => {
      outerDOM.classList.add('vid-grab--focus');
    },
    deselectNode: () => {
      outerDOM.classList.remove('vid-grab--focus');
    },
  };
};
